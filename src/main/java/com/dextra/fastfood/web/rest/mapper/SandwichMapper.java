...
public class SandwichMapper implements Mapper<Sandwich, SandwichDto>
{
        ...
        if (dto.getIngredients() != null) {
            ...
            dto.getIngredients().forEach((k, v) ->
                sandwich.getSandwichIngredients()
                    .add(new SandwichIngredient(null, ZonedDateTime.now(), ZonedDateTime.now(), sandwich, ingredientRepository.getOne(k), v)));
    ...
    public SandwichDto toDto(Sandwich dataObject)
    {
        ...
        SandwichDto dto = SandwichDto.builder().id(dataObject.getId()).name(dataObject.getName()).build();
