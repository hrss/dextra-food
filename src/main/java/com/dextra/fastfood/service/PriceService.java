...
public class PriceService {
   ...
   */
  @Autowired
  public PriceService(final SandwichRepository sandwichRepository,
  ...
  public BigDecimal getSandwichPrice(Sandwich sandwich) throws InvalidSandwichException {
    ...
    if (sandwich.getId() != null && this.sandwichRepository.findById(sandwich.getId())
        .isPresent()) {
  ...
  private BigDecimal checkOffers(Map<Ingredient, Long> ingredients, BigDecimal price) {
    ...
    if (ingredients.containsKey(cheese)) {
      if (ingredients.get(cheese) > 2) {
        price = price
            .subtract(cheese.getPrice().multiply(new BigDecimal(ingredients.get(cheese) / 3)));
      }
    }
  ...
  private BigDecimal getPrice(Map<Ingredient, Long> ingredients) {
    ...
    for (Ingredient ingredient : ingredients.keySet()) {
      price = price
          .add(ingredient.getPrice().multiply(new BigDecimal(ingredients.get(ingredient))));
    }
    return price;
  }
